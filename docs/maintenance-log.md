...
## 2026-09-13：藏书阁多阶段入场过渡

- 将首页“叩门”后的直接切换改为约 2.8 秒的连续过场：门扉回应、镜头推进、双门开启、藏书长廊穿行、薄雾掠过和内阁显现。
- 根据实机视觉反馈重做过场门体，补充匾额、门框、立柱、门槛、格栅、铜钉、门牌与门环，使其呈现明确的中式藏书阁双扇门结构。
- 移除过亮的圆形光斑与放射流光，门后改为低亮度的矩形空间透视；加入顶梁、地面纵深线、两侧木墙、远端书架、四盏暖灯和浮尘。
- 内阁出现后，标题、分类导航、书架和书卷按层级依次落位；动画完成后恢复原有悬浮提示与展卷交互。
- 动画期间暂时锁定页面滚动，并用三个阶段状态向辅助技术说明进度。
- 遵循系统“减少动态效果”设置：该模式下使用约 120 毫秒的快速切换，不播放位移和缩放动画。
